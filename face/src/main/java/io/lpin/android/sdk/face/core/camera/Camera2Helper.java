...
public class Camera2Helper {
    ...
    private CameraDevice.StateCallback mDeviceStateCallback = new CameraDevice.StateCallback() {
        ...
        public void onOpened(@NotNull CameraDevice cameraDevice) {
            ...
            if (camera2Listener != null) {
                getCameraOri(rotation, mCameraId);
                camera2Listener.onCameraOpened(cameraDevice, mCameraId, mPreviewSize, mSensorOrientation, isMirror);
            }
        }
    ...
    public static void getYUVBytes(YUV yuv, Image.Plane[] planes) {
        if (yuv.y == null) {
            yuv.y = new byte[planes[0].getBuffer().limit() - planes[0].getBuffer().position()];
            yuv.u = new byte[planes[1].getBuffer().limit() - planes[1].getBuffer().position()];
            yuv.v = new byte[planes[2].getBuffer().limit() - planes[2].getBuffer().position()];
        }
        planes[0].getBuffer().get(yuv.y);
        planes[1].getBuffer().get(yuv.u);
        planes[2].getBuffer().get(yuv.v);
    }
        ...
        public void onImageAvailable(ImageReader reader) {
            try {
                ...
                if (camera2Listener != null && image.getFormat() == ImageFormat.YUV_420_888) {
                    // 이미지 프로세싱 중이면 전환
                    if (camera2Listener.isProcessingFrame()) {
                        image.close();
                        return;
                    }
                    lock.lock();
                    camera2Listener.onPreview(image, mPreviewSize);
                    lock.unlock();
                }
            } catch (Exception ex) {
                ex.printStackTrace();
