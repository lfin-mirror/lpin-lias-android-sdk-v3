...
- 선택적 앱 서명 인증서 SHA-256 (`signing_cert_sha256`)
- 선택적 허용 기능 목록 (`features`)
...
이 키는 고객사 앱의 패키지명과 사용 가능 기간을 기준으로 발급되며, 필요 시 서명 인증서와 허용 기능까지 포함할 수 있습니다.
...
`signing_cert_sha256`를 포함해서 발급받는 경우, 실제 앱 서명 인증서 SHA-256이 라이센스와 일치해야 합니다.
...
compact license key에 `features`가 포함된 경우, 고객사 앱은 발급받은 기능만 사용할 수 있습니다.
...
6. `signing_cert_sha256`가 있는 경우 앱 인증서 일치 여부 확인
7. `features`가 있는 경우 요청 기능 허용 여부 확인
...
- [ ] `signing_cert_sha256`를 사용하는 경우 실제 앱 서명 인증서 SHA-256이 일치하는가
- [ ] `features`를 사용하는 경우 사용하려는 기능(`face`, `scanner`, `space`, `pixel-matching`)이 포함되어 있는가
...
- 릴리즈 서명 키가 변경되면 기존 라이센스는 더 이상 유효하지 않을 수 있습니다.
...
3. SDK AAR 빌드
4. 고객사 앱 정보(package name, signing cert SHA-256, features, 기간)로 compact license key 생성
